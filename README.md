...
> - **인앱 자동 업데이트** — 24시간마다 GitHub Releases를 확인해 새 버전이 있으면 알림 표시줄에서 다운로드(진행률 표시) → 시스템 인스톨러로 한 번에 설치. 특정 버전은 "이 버전 건너뛰기"로 자동 알림에서 제외 가능(수동 확인 시엔 항상 다시 표시). 설정 → 유용한 기능 과 About 화면에서 즉시 확인 가능
> - **설정 → 유용한 기능 카테고리 신설** — 인앱 업데이터, 화이트보드 동기화/공유, 학습 효과음(SFX) 등 fork 전용 설정을 한곳에 모아서 확인
